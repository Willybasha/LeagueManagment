$(document).ready(function () {
    ...
    datatable = $('#Team').DataTable({
        ...
        columns: [
            { "data": "teamid", "name": "TeamId", "className": "d-none" },
            {
                "name": "",
                "className": "d-flex align-items-center",
                "render": function (data, type, row) {
                    return `<div class="symbol symbol-50px overflow-hidden me-3">
                                                <a href="/Tournament/Details/${row.id}">
                                                    <div class="symbol-label h-70px">
                                                        <img src="${(row.Logo === null ? '/images/Teams/4453e046-e2f2-4d3e-bdf8-a5c07c343f88.jpg' : row.Logo)}" alt="cover" class="w-100">
                                                    </div>
                                                </a>
                                            </div>
                                            <div class="d-flex flex-column">
                                                <a href="/Books/Details/${row.id}" class="text-primary fw-bolder mb-1">${row.name}</a>
                                            </div>`;
                }
            },
            { "data": "officialwebsiteurl", "name": "OfficialWebsiteUrl" },
            {
                "name": "FoundationDate",
                "render": function (data, type, row) {
            ...
            { "data": "tournaments", "name": "Tournaments", "orderable": false }, 
            {
                "className": 'text-end',
                "orderable": false,
                "render": function (data, type, row) {
                    return `<a href="#" class="btn btn-light btn-active-light-primary btn-sm" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end">
                            ...
                            <div class="menu-item px-3">
                                <a href="/Team/Edit/${row.id}" class="menu-link px-3">
                                    Edit
                                </a>
                            </div>
                            <!--end::Menu item-->
